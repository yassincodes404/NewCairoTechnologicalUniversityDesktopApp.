...
bool AuthService::login(const std::string& username, const std::string& password, User& outUser) {
    ...
    sql << "SELECT id, username, role, linked_student_id FROM users WHERE username = '" << username << "' AND password_hash = '" << password << "';";
...
std::optional<User> AuthService::getUserByUsername(const std::string& username) {
    ...
    std::stringstream sql;
    sql << "SELECT id, username, role, linked_student_id FROM users WHERE username = '" << username << "';";
