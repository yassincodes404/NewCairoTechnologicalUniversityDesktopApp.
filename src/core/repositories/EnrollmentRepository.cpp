...
std::vector<Enrollment> EnrollmentRepository::getAll() {
    ...
    std::string sql = "SELECT id, student_id, course_id, semester, year, grade FROM enrollments;";
...
std::vector<Enrollment> EnrollmentRepository::getByStudentId(int studentId) {
    ...
    sql << "SELECT id, student_id, course_id, semester, year, grade FROM enrollments WHERE student_id = " << studentId << ";";
...
std::vector<std::pair<Course, Enrollment>> EnrollmentRepository::getByStudentIdWithCourses(int studentId) {
    ...
    std::stringstream sql;
    sql << "SELECT e.id, e.student_id, e.course_id, e.semester, e.year, e.grade, "
...
std::optional<Enrollment> EnrollmentRepository::getById(int id) {
    ...
    std::stringstream sql;
    sql << "SELECT id, student_id, course_id, semester, year, grade FROM enrollments WHERE id = " << id << ";";
...
bool EnrollmentRepository::insert(const Enrollment& enrollment) {
    std::stringstream sql;
    sql << "INSERT INTO enrollments (student_id, course_id, semester, year, grade) VALUES ("
